...
class ErrorCorrection:
    def __init__(self, encoding: str, version: int, correction_level: str):
        self.required_bits: int = self.__find_required_bits_number(
            correction_level, version
        )
        ...
        self.word_blocks: list[str] = self.__create_blocks(
            self.padded_encoding, correction_level, version
        )
        self.error_blocks: list[str] = self.__create_blocks(
            self.error_encoding, correction_level, version
        )
    ...
    def __structure_words(self, version: int):
        ...
        max_word = max(
            max(map(len, self.word_blocks)), max(map(len, self.error_blocks))
        )
        ...
        for block1 in range(numbers.blocks_group_1):
            ...
            blocks.append(
                self.__split_into_words(
                    encoding[start : start + (numbers.dc_group_1 * 8)]
                )
            )

        for block2 in range(numbers.blocks_group_2):
            start = (
                (numbers.blocks_group_1 * numbers.dc_group_1)
                + (block2 * numbers.dc_group_2)
            ) * 8
            blocks.append(
                self.__split_into_words(
                    encoding[start : start + (numbers.dc_group_2 * 8)]
                )
            )
